...
export async function analyzeConfigsPerPackage(configEntries: Array<{package: string, configs?: Array<{source: string, destination: string}>, sourceFile?: string, sourceType?: string, groupName?: string}>) {
  ...
  console.log("Configuration files to sync:");

  // First pass: analyze all packages to see if we can show a summary
  const packagesWithChanges: Array<{package: string, configs?: Array<{source: string, destination: string}>, sourceFile?: string, sourceType?: string, groupName?: string}> = [];
  const packagesUpToDate: Array<{package: string, configs?: Array<{source: string, destination: string}>, sourceFile?: string, sourceType?: string, groupName?: string}> = [];
  const packagesWithConflicts: Array<{package: string, configs?: Array<{source: string, destination: string}>, sourceFile?: string, sourceType?: string, groupName?: string}> = [];
  ...
  for (const entry of packagesWithConfigs) {
    ...
    if (hasConflicts) {
      packagesWithConflicts.push(entry);
    } else if (hasChanges) {
      packagesWithChanges.push(entry);
    } else {
      packagesUpToDate.push(entry);
    }
  }
  ...
  for (const entry of allPackagesToProcess) {
    ...
    const sourcePrefix = formatPackageSource({
...
async function showDotfilesSummary(packages: Array<{package: string, configs?: Array<{source: string, destination: string}>, sourceFile?: string, sourceType?: string, groupName?: string}>) {
  ...
  if (packageCount === 1) {
    ...
    const summarySpinner = spinner("  Dotfiles - checking...", { enabled: true });
    summarySpinner.stop("");
    console.log();
  } else {
