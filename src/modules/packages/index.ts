...
import { DEFAULT_PROTECTED_PACKAGES } from "../../utils/constants";
import { getInstalledVersion as queryInstalledVersion } from "../pacman/query";
...
// DEFAULT_PROTECTED_PACKAGES is now sourced from constants.ts
...
function saveManagedLock(lock: ManagedLock): void {
  ...
  const home = getHomeDirectory();
  const stateDir = join(home, '.owl', '.state');
  const lockPath = join(stateDir, 'managed.lock');
  const data = JSON.stringify(lock, null, 2);
  // Atomic write with simple lock to avoid concurrent corruption
  // Intentionally sync API for simplicity in critical path
  const run = async () => atomicWriteFile(lockPath, data);
  // Fire and forget since function is sync; ensure lock around write
  // eslint-disable-next-line @typescript-eslint/no-floating-promises
  withFileLock(stateDir, 'managed', run).catch(() => atomicWriteFile(lockPath, data));
}

async function getInstalledVersion(packageName: string): Promise<string | undefined> {
  return queryInstalledVersion(packageName);
}
...
export async function planPackageActions(packageNames: string[]): Promise<PackageAction[]> {
...
export async function isPackageNewer(installedVersion: string, availableVersion: string): Promise<boolean> {
  const { isPackageNewer: qIsNewer } = await import("../pacman/query");
  return qIsNewer(installedVersion, availableVersion);
}
...
async function installPackagesWithStreaming(packages: string[]): Promise<void> {
  ...
  for (const pkg of packages) {
    ...
    try {
      await manager.installPackage(pkg, false, (message: string) => {
