...
export class PacmanManager {
  ...
  /**
   * Install a single package. Optionally provide a progress callback.
   */
  ...
  async installPackageWithProgress(packageName: string, verbose = false, progressCallback: ProgressCallback | null = null): Promise<void> {
    return this.installPackage(packageName, verbose, progressCallback);
  }
    ...
    try {
      ...
      if (groupOutput.trim()) {
        ...
        results.unshift({
          ...
          repository: "core",
          installed: await qIsInstalled(searchTerm),
          inConfig: false
        });
      }
    } catch {
  ...
  async isPackageInstalled(packageName: string): Promise<boolean> {
    return qIsInstalled(packageName);
  }
  ...
  async isPackageGroupInstalled(groupName: string): Promise<boolean> {
    return qIsGroupInstalled(groupName);
  }
